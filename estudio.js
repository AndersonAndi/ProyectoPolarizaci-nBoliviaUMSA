...
// ==============================
// ==============================
//   SECCIÓN 6.5: SIMULACIÓN
// ==============================

// Comentarios cargados de CSV (los obtendremos de la Sección 6)
let listaComentarios = [];   // texto del comentario
let listaCategorias = [];    // 1, 0, -1

// Copiar datos de la sección 6 cuando termina el fetch()
function inicializarSimulacion(filasCSV) {
  listaComentarios = [];
  listaCategorias = [];

  filasCSV.forEach((col) => {
    const texto = col[0]?.trim();  // columna 0 = comentario original
    const cat = col[8]?.trim();    // categoría
  ...
  // Mezclar ambas listas en paralelo
  mezclarComentarios();

  // Llenar contadores iniciales
  document.getElementById("sim-count-pos").textContent = countPos;
  document.getElementById("sim-count-neu").textContent = countNeu;
  document.getElementById("sim-count-neg").textContent = countNeg;
  document.getElementById("sim-total").textContent =
    countPos + countNeu + countNeg;
}

// Mezclar en paralelo
function mezclarComentarios() {
  for (let i = listaComentarios.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));

    [listaComentarios[i], listaComentarios[j]] =
      [listaComentarios[j], listaComentarios[i]];

    [listaCategorias[i], listaCategorias[j]] =
      [listaCategorias[j], listaCategorias[i]];
  }
}

// DOM
const btnSiguiente = document.getElementById("btnSiguienteComentario");
const contenedorLeidos = document.getElementById("comentarios-leidos");
const simStatus = document.getElementById("sim-status");

let totalLeidos = 0;
let destructivosLeidos = 0;
let neutralesLeidos = 0;
let constructivosLeidos = 0;

// Lógica: leer comentario sin reemplazo
btnSiguiente.onclick = () => {
  if (listaComentarios.length === 0) {
    simStatus.textContent = "¡Ya no quedan comentarios!";
    btnSiguiente.disabled = true;
    return;
  }

  // Extraer primero
  const comentario = listaComentarios.shift();
  const categoria = listaCategorias.shift();

  // Crear DIV para mostrarlo
  const item = document.createElement("div");
  item.classList.add("comentario-item");

  if (categoria === "1" || categoria === "1.0") item.classList.add("pos");
  else if (categoria === "0" || categoria === "0.0") item.classList.add("neu");
  else item.classList.add("neg");

  item.textContent = comentario;
  contenedorLeidos.appendChild(item);

  // Actualizar contadores
  if (categoria === "1" || categoria === "1.0") countPos--;
  else if (categoria === "0" || categoria === "0.0") countNeu--;
  else countNeg--;

  totalLeidos++;
  if (categoria === "1" || categoria === "1.0") destructivosLeidos++;
  else if (categoria === "0" || categoria === "0.0") neutralesLeidos++;
  else constructivosLeidos++;

  const totalRestante = countPos + countNeu + countNeg;

  document.getElementById("sim-count-pos").textContent = countPos;
  document.getElementById("sim-count-neu").textContent = countNeu;
  document.getElementById("sim-count-neg").textContent = countNeg;
  document.getElementById("sim-total").textContent = totalRestante;

  document.getElementById("sim-leidos").textContent = totalLeidos;
  document.getElementById("sim-leidos-des").textContent = destructivosLeidos;
  document.getElementById("sim-leidos-neu").textContent = neutralesLeidos;
  document.getElementById("sim-leidos-const").textContent = constructivosLeidos;

  actualizarProbabilidades();

  simStatus.textContent = "Comentario leído.";

  if (totalRestante === 0) {
    btnSiguiente.disabled = true;
    simStatus.textContent = "¡Se leyeron todos los comentarios!";
  }
};


// ================================================
// INTEGRACIÓN CON LA SECCIÓN 6 (CSV ya leído)
// ================================================



function actualizarProbabilidades() {
  const pos = parseInt(document.getElementById("sim-count-pos").textContent);
  const neu = parseInt(document.getElementById("sim-count-neu").textContent);
  const neg = parseInt(document.getElementById("sim-count-neg").textContent);
  const total = pos + neu + neg;

  const pPos = total ? ((pos / total) * 100).toFixed(4) : 0;
  const pNeu = total ? ((neu / total) * 100).toFixed(4) : 0;
  const pNeg = total ? ((neg / total) * 100).toFixed(4) : 0;

  document.getElementById("sim-prob-pos").textContent = `${pPos}%`;
  document.getElementById("sim-prob-neu").textContent = `${pNeu}%`;
  document.getElementById("sim-prob-neg").textContent = `${pNeg}%`;
  document.getElementById("sim-prob-total").textContent = `100%`;
}
...
document.getElementById("btnCalcularHG").onclick = () => {
  ...
  if (x1 > K1 || x2 > K2 || x3 > K3) {
    alert("Los valores de muestra no pueden superar la cantidad disponible.");
    return;
  }

  // calcular combinatorias
  let C1 = nCk(K1, x1);
  let C2 = nCk(K2, x2);
  let C3 = nCk(K3, x3);
  let CT = nCk(N, n);
  ...
  // mostrar fórmula reemplazada
  document.getElementById("salida-formula").innerHTML = `
  ...
  document.getElementById("salida-resultado").innerHTML = `
    <h4>Resultado</h4>
    <p><strong>Fracción:</strong> ${C1 * C2 * C3} / ${CT}</p>
    <p><strong>Decimal:</strong> ${P.toFixed(10)}</p>
    <p><strong>Porcentaje:</strong> ${(P * 100).toFixed(6)}%</p>
  `;
